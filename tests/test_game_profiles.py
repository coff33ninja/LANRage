...
class TestGameProfiles:
    ...
    def test_profile_has_required_fields(self, profile_path):
        ...
        required_fields = {
            "name", "executable", "ports", "protocol", "broadcast",
            "multicast", "keepalive", "mtu", "description",
    ...
    def test_profile_field_types(self, profile_path):
        ...
        for game_id, game_data in profile.items():
            ...
            assert isinstance(game_data["executable"], str), f"{game_id}: executable must be string"
            assert isinstance(game_data["protocol"], str), f"{game_id}: protocol must be string"
            assert isinstance(game_data["description"], str), f"{game_id}: description must be string"
            assert isinstance(game_data["packet_priority"], str), f"{game_id}: packet_priority must be string"
            
            # List fields
            assert isinstance(game_data["ports"], list), f"{game_id}: ports must be list"
            assert all(isinstance(p, int) for p in game_data["ports"]), f"{game_id}: ports must be integers"
            
            # Boolean fields
            assert isinstance(game_data["broadcast"], bool), f"{game_id}: broadcast must be boolean"
            assert isinstance(game_data["multicast"], bool), f"{game_id}: multicast must be boolean"
            assert isinstance(game_data["low_latency"], bool), f"{game_id}: low_latency must be boolean"
            assert isinstance(game_data["high_bandwidth"], bool), f"{game_id}: high_bandwidth must be boolean"
            
            # Integer fields
            assert isinstance(game_data["keepalive"], int), f"{game_id}: keepalive must be integer"
    ...
    def test_profile_field_values(self, profile_path):
        ...
        for game_id, game_data in profile.items():
            ...
            assert game_data["protocol"] in ["udp", "tcp", "both"], \
                f"{game_id}: protocol must be 'udp', 'tcp', or 'both'"
            
            # Packet priority must be valid
            assert game_data["packet_priority"] in ["low", "medium", "high"], \
    ...
    def test_total_game_count(self):
        ...
        assert total_games >= 75, f"Only {total_games} games found, expected at least 75"
